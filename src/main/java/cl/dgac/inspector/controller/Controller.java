...
@RestController
@RequestMapping("api/v1.0/Inspector")
public class Controller {
    ...
    public ResponseEntity<List<DtoModel>> listar(){
        List<Modelo> lista = servicio.listarInspector();
        return new ResponseEntity<List<DtoModel>>(Mapper.lsiatasDto(lista),HttpStatus.OK);
    } 
    @GetMapping("/bucador")
    ...
    @RequestParam(name= "apellido",required = false) String apellido) {
        if ((nombre == null || nombre.isBlank()) && (apellido == null ||apellido.isBlank()) ){
            ...
            return new ResponseEntity<List<DtoModel>>(Mapper.lsiatasDto(servicio.filtarPorApellido(apellido)),HttpStatus.OK);
        }else if (apellido == null ||apellido.isBlank()){
            return new ResponseEntity<List<DtoModel>>(Mapper.lsiatasDto(servicio.filtarpornombre(nombre)),HttpStatus.OK);
        }else 
            return new ResponseEntity<List<DtoModel>>(Mapper.lsiatasDto(servicio.filtarnombreYapelldio(nombre, apellido)), HttpStatus.OK);
    }
    @GetMapping("/bucarRut")
    public ResponseEntity<DtoModel> filtradoPorRut(@RequestParam(name="rut") String rut){
        return new ResponseEntity<DtoModel>(Mapper.modelToDto(servicio.filtaRut(rut)),HttpStatus.OK);
    }
    @PostMapping("/nuevoInspector")
    public ResponseEntity<DtoModel> addinspector(@Valid@RequestBody DtoModel entity) {
        return new ResponseEntity<DtoModel>(Mapper.modelToDto(servicio.save(Mapper.addModelo(entity))),HttpStatus.OK);
    }
    @PutMapping("/editar")
    public ResponseEntity<DtoModel> editarInspector(@RequestParam(name= "rut") String rut,@Valid @RequestBody DtoModel entity) {
        Long id = servicio.filtaRut(rut).getId();
